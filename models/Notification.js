...
NotificationSchema.pre("save", function (next) {
  if (!this.expiresAt) {
    const daysToExpire =
      this.priority === "urgent" ? 7 : this.priority === "high" ? 14 : 30;
...
NotificationSchema.methods.completeAction = function (actionIndex, adminId, notes) {
  if (this.actions[actionIndex]) {
    ...
    // Auto-resolve if all actions completed
    const allCompleted = this.actions.every(a => a.isCompleted);
    if (allCompleted && this.autoResolve) this.isActive = false;
  }
  return this.save();
};

NotificationSchema.methods.addRecipient = function (adminId, role) {
  const exists = this.recipients.some(r => r.adminId?.toString() === adminId.toString());
  if (!exists) this.recipients.push({ adminId, role });
  return this.save();
};
...
NotificationSchema.statics.getActiveNotifications = function (adminId, filters = {}) {
  const query = {
    isActive: true,
    $or: [{ targetAudience: "all_admins" }, { "recipients.adminId": adminId }],
    expiresAt: { $gt: new Date() },
  };
  if (filters.type) query.type = filters.type;
  if (filters.priority) query.priority = filters.priority;
  if (filters.category) query.category = filters.category;
  if (filters.unreadOnly) query["recipients.isRead"] = false;

...
const CustomerSchema = new mongoose.Schema(
  {
    ...
    address: {
      ...
      country: { type: String, default: "New Zealand", trim: true },
    ...
    notes: {
      ...
      customerServiceNotes: { type: String, maxlength: 2000 },
    },
...
CustomerSchema.pre("save", function (next) {
  ...
  // Upgrade verification level automatically
  const approvedDocs = this.verification.documentsSubmitted.filter(
    (doc) => doc.status === "approved"
  );
  if (
    approvedDocs.length >= 5 &&
    ["none", "basic"].includes(this.verification.verificationLevel)
  ) {

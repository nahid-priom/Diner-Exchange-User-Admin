...
/**
 * Audit Log Schema - Tracks all sensitive and important admin actions for compliance and traceability.
 */
...
// --- Risk Score Calculation Middleware ---
AuditLogSchema.pre("save", function (next) {
  ...
  if (highRisk.includes(this.action)) score += 50;
  else if (mediumRisk.includes(this.action)) score += 30;
  else score += 10;

  if (this.status === "failed") score += 20;
  if (this.severity === "critical") score += 30;
  else if (this.severity === "high") score += 20;

  const hour = new Date().getHours();
  if (hour >= 22 || hour <= 6) score += 15; // Off-hours action
